...
'''
        ...
        if str(ver) == str(self.version):
            ...
            while True:
                try:
                    r = requests.get('https://raw.githubusercontent.com/Cristi24135123512351512/pascfndeluxe/main/PascFN.py', timeout=10)
                    with open(f'PascFN{ver}.py', 'w', encoding='utf8') as file:
                        file.write(r.text)
                    break
                except Exception as e:
                    print(f'{Fore.RED}Error downloading new version {e}')
            os.system('cls')
            os.system(f'py PascFN{ver}.py')
        from time import time
        system('cls')
        windll.kernel32.SetConsoleTitleW(
                f'PascFN | v{self.version} | Select a mode')
        print(self.t)
        print(f'''
...
''')
        ...
        if mode == "5":
            ...
            print(self.t)
            windll.kernel32.SetConsoleTitleW(
                f'PascFN V{self.version} | Module: ComboEditor')
            print(f'''
...
''')
            ...
            windll.kernel32.SetConsoleTitleW(
                f'PascFN | v{self.version} | Scraping Proxies | BoltProxy')
            ...
            while True:
                ...
                if len(urls) > 0:
                    ...
                    break
            windll.kernel32.SetConsoleTitleW(
                f'PascFN | v{self.version} | Done Scraping | BoltProxy')
                ...
                while True:
                    try:
                        if Checker.mode.lower() == 'bolt':
                            self.cuitype = 'bolt'
                            print(Fore.MAGENTA)
                            print_slowly('Using PascFN CUI', 0.05)
                            break
        ...
        print(f'{Fore.LIGHTCYAN_EX}[PascFN] Starting Threads...\n')
        windll.kernel32.SetConsoleTitleW(
            f'PascFN | v{self.version} | Getting ready!')
        ...
        print(f'{Fore.GREEN}[PascFN] Done!\n')
        sleep(3)
        system('cls')
        print(self.t)
        if not Counter.proxy:
